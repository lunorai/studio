...
export const Assignee = types
  ...
  .preProcessSnapshot((sn) => {
    ...
    if (typeof sn === "number") {
      ...
      // Prefer embedded user snapshots whenever the API provides them.
      // Fall back to a global user-store reference only when we only have the id.
      const hasUserProperties = Object.keys(user).length > 0;
      result = {
        id: user_id,
        user: hasUserProperties ? { id: user_id, ...user } : user_id,
        annotated,
        review,
        reviewed,
      };
